...
你是 API 对接专家。**任何时候**遇到 `openapi.json` / `swagger.json` / 接口规范文件（JSON 格式），**永远不要直接 Read 整个文件**——文件可能数千行且含有大量 `$ref` 引用——始终使用 `openapi` 按需查询。

**也支持远程 URL** — 直接传入 `https://` 链接即可自动抓取 OpenAPI 规约，无需先下载到本地。

**开发新 API 或修改接口时**，使用 `openapi` 写入子命令同步更新接口文档，保持代码与文档一致。
...
```bash
<skill_dir>/bin/openapi-<OS>-<ARCH>  <command> <spec_file> [options]
```

`<skill_dir>` 为该 SKILL.md 所在目录的绝对路径。**单文件二进制，零依赖，直接运行。**
...
> 在 opencode 中调用时，根据 `runtime.GOOS` 和 `runtime.GOARCH` 拼接二进制路径：
> `<skill_dir>/bin/openapi-${GOOS}-${GOARCH}`（Windows 需追加 `.exe`）

**支持远程 URL**：直接传入 `https://...` 地址即可自动抓取 OpenAPI 规约：

```bash
openapi list https://petstore.swagger.io/v2/swagger.json --limit 10
openapi endpoint https://petstore.swagger.io/v2/swagger.json --path /pet --method post
```
...
```bash
# 查看所有接口（方法 + 路径 + 摘要）
openapi list api.json

# 按关键词搜索（匹配路径、摘要、描述、Tag）
openapi search api.json "user"

# 按 Tag 筛选
openapi tag api.json Users

# 分组查看（按 Tag 分组）
openapi list api.json --group-by-tag

# 限制输出条数（防止大文件卡死）
openapi list api.json --limit 20

# 紧凑模式（每接口一行）
openapi list api.json --compact
```
...
openapi endpoint api.json --path /users --method post

# 获取某个 Schema/Definition 定义
openapi schema api.json User

# 查看 API 基本信息
openapi info api.json

# 限制 Schema 展开深度（防止深嵌套卡死）
openapi endpoint api.json --path /users --method post --depth 3

# 只输出指定部分
openapi endpoint api.json --path /users --method post --params-only
openapi endpoint api.json --path /users --method post --request-only
openapi endpoint api.json --path /users --method post --response-only
openapi endpoint api.json --path /users --method post --fields-only

# 批量查询多个路径
openapi endpoint api.json --paths /users,/roles --method get
```
...
| 命令 | 语法 | 用途 |
|------|------|------|
| `list` | `openapi list <spec> [options]` | 列出所有接口 |
| `info` | `openapi info <spec> [--output json]` | API 基本信息 |
| `search` | `openapi search <spec> <keyword> [options]` | 按关键词搜索接口 |
| `tag` | `openapi tag <spec> <tag> [options]` | 按 Tag 列出接口 |
| `endpoint` | `openapi endpoint <spec> --path P --method M [options]` | 获取接口详情 |
| `schema` | `openapi schema <spec> <name> [options]` | 获取 Schema 定义 |
| `uses-field` | `openapi uses-field <spec> <field> [options]` | 查找引用某字段的接口 |
...
| 命令 | 语法 | 用途 |
|------|------|------|
| `upsert-endpoint` | `openapi upsert-endpoint <spec> --path P --method M [options]` | 新增或更新接口 |
| `remove-endpoint` | `openapi remove-endpoint <spec> --path P [--method M]` | 删除接口 |
| `upsert-schema` | `openapi upsert-schema <spec> --name N --file F` | 新增或更新 Schema |
...
开发新 API 时，使用 `openapi` 同步更新 `openapi.json`。
...
# 2. 写入 spec
openapi upsert-endpoint openapi.json --path /api/AgentGroups --method POST --file "$env:TEMP\new_endpoint.json"
```

方式二：通过命令行参数（简单接口）

```bash
openapi upsert-endpoint openapi.json \
...
openapi upsert-schema openapi.json --name MyDto --file "$env:TEMP\schema.json"
```
...
```bash
# 删除单个方法
openapi remove-endpoint openapi.json --path /api/AgentGroups --method DELETE

# 删除整个路径的所有方法
openapi remove-endpoint openapi.json --path /api/old-endpoint
```
...
1. **永远不要直接 Read OpenAPI 规范文件**，始终使用 `openapi` 查询
